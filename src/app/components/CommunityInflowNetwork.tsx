...
/** パネル用のハブのアイコン（図のハブと同じく、画像が無ければ名前の頭 2 文字） */
const HubAvatar: React.FC<{ hub: InflowHub; broken: boolean; othersLabel: string }> = ({
  hub,
  broken,
  othersLabel,
}) => {
  ...
  if (hub.isOthers) {
    return (
      <span
        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-[10px] text-[#2C2C2A]"
  ...
  return (
    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border bg-muted text-sm">
      {initialsOf(hub.name)}
    </span>
  );
};
...
const CommunityInflowNetwork: React.FC<CommunityInflowNetworkProps> = ({ lang, issuer }) => {
  ...
  const openCreator = async (hub: InflowHub) => {
    ...
    try {
      ...
      if (pick) {
        router.push(collectionPath(lang, { issuer: hub.issuer, taxon: String(pick.taxon) }));
        return;
      }
    } catch (error) {
  ...
  if (!inflow || !layout || inflow.owners.length === 0) {
    return <div className="py-8 text-center text-sm text-gray-500">{community.status.noData}</div>;
  }
        ...
        >
          ...
          {focusedOwner && (
            <div className="absolute left-2 top-2 z-10 w-64 max-w-[calc(100%-3.5rem)] space-y-2 rounded-md border bg-background/95 p-3 text-sm shadow-md">
              ...
              <div className="space-y-0.5 text-xs text-muted-foreground">
                <div>{community.tooltip.home}: {focusedOwner.homeName}</div>
  ...
  {hoveredOwner.homeName}
              ...
              {hoveredHub && (
                <div className="space-y-1">
                  <div className="font-medium break-words">{hubLabel(hoveredHub)}</div>

...
import { AddressGroup } from '@/utils/db';
...
export const OwnerDetailSheet: React.FC<OwnerDetailSheetProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (owner) {
        // 【既存編集モード】
        setEditedData(owner);
        setIsEditing(false); // まずは詳細表示
      } else {
        // 【新規作成モード】
        const baseData = {
          name: '',
          addresses: initialAddresses, // initialAddressesを初期値にセット
          xAccount: null,
          memo: null
        };
        setEditedData(baseData);
        setIsEditing(true); // 新規の場合は最初から編集モード
      }
      setNewAddress('');
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (owner) {
        // 更新
        await updateAddressGroup(dataToSave as AddressGroup);

        // 拡張機能に通知
        window.postMessage({ type: 'OWNERNOTE_UPDATED' }, '*');
      } else {
        // 新規作成
        await createAddressGroup(dataToSave as Omit<AddressGroup, 'id' | 'updatedAt'>);

        // 拡張機能に通知
        window.postMessage({ type: 'OWNERNOTE_UPDATED' }, '*');
        
        onOpenChange(false); // 新規作成後はシートを閉じる
      }
      
      await onSave(); // リストの再読み込み
      setIsEditing(false);
    } catch (error) {

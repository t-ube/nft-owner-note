...
class DatabaseManager {
  ...
  // NFT Detail Methods
  async updateNFTDetail(detail: Omit<NFTDetail, 'id' | 'updatedAt'>): Promise<NFTDetail> {
    const db = await this.initDB();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('nftDetails', 'readwrite');
      const store = transaction.objectStore('nftDetails');

      const now = Date.now();
      const completeDetail: NFTDetail = {
        id: crypto.randomUUID(),
        updatedAt: now,
        ...detail
      };

      const request = store.put(completeDetail);

      request.onerror = () => reject(request.error);
      request.onsuccess = () => resolve(completeDetail);
    });
  }

  async getNFTDetailsByProjectId(projectId: string): Promise<NFTDetail[]> {
    const db = await this.initDB();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('nftDetails', 'readonly');
      const store = transaction.objectStore('nftDetails');
      const index = store.index('projectId');
      const request = index.getAll(projectId);
  ...
  async getNFTDetailByNftId(nftId: string): Promise<NFTDetail | undefined> {
    const db = await this.initDB();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('nftDetails', 'readonly');
      const store = transaction.objectStore('nftDetails');
      const index = store.index('nftId');
      const request = index.get(nftId);
  ...
  async deleteNFTDetailsByProjectId(projectId: string): Promise<void> {
    const db = await this.initDB();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('nftDetails', 'readwrite');
      const store = transaction.objectStore('nftDetails');
